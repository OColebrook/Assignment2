...
I implimented this by creating an additional called selectedbutton to store the button assiciated with the pokemon whose details are currently showing.  I initialised the variable with thre value set to my default pokemon and then applied the CSS class selected to this button. 

When a new pokemon is selected (button clicked) the "selected" CSS is removed from the classList property of the current selected pokemon button and then the value of selectedbutton is reset to the newly selected button and the classlist property of this button is updated to include the "selected" CSS class.

I used this approach because it was the cleanest and  most straight forward option I had in mind and it was partly demonstrated in module 4.
